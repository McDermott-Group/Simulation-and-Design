function steadyStateModelDifferentCph
%steadyStateModelDifferentCph Explore the quasiparticle steady-state
...
for krqp = 1:length(c)
    ...
    parfor kV = 1:length(V)
        ...
        [~, ~, ~, ~, n_qp, ~, P(kV, krqp)] =...
            ...
            Vsim, r_direct, r_phonon, 0, csim, vol, N);
...
legends = cell(0);
for k = 1:length(c)
    legends{k} = ['c^{res} = ', num2str(c(k), '%.2e')];
end
