...
void CPW(void)
{
	...
	//	ptArray[1] = LPoint_Set(xCenter,yCenter);
		...
		OR = IR + wireWidth;
		AR = (OR + IR)/2 / Scale;
	  	meanderLength = meanderWidth/Scale + Pi*AR;
		numMeander = cpwLength / meanderLength;
	  	float newWireOffSetX = 3*wireWidth + 2*gpSpacing + meanderSpace;
	  	float startAngle, endAngle;
	  	int n=0;
	  	LCoord groundWireOffSet;
	  	groundWireOffSet = wireWidth + gpSpacing;
	  	//Place the wires for the center trace
	  	/***********************************************************************************************/
	  	for(n = 0; n < numMeander-1; n++)
	  	{
	  		//Center Trace
			centerX = xCenter + n*newWireOffSetX;
	  		centerY = yCenter;
	  		//Ground Wire 1,2
	  		centerX1 = centerX + groundWireOffSet;
	  		centerX2 = centerX - groundWireOffSet;
			//Ground Wire 1,2
			centerY1 = centerY;
			centerY2 = centerY;
	  		
	  		xOffSet = wireWidth / 2;
	  		yOffSet = meanderWidth / 2;
	  		
	  		//Center Trace
	  		leftCornerX = centerX - xOffSet;
	  		leftCornerY = centerY - yOffSet;
	  		rightCornerX = centerX + xOffSet;
	  		rightCornerY = centerY + yOffSet;
	  		//GoundWire 1
	  		leftCornerX1 = centerX1 - xOffSet;
	  		leftCornerY1 = centerY1 - yOffSet;
	  		rightCornerX1 = centerX1 + xOffSet;
	  		rightCornerY1 = centerY1 + yOffSet;
	  		//GroundWire 2
	  		leftCornerX2 = centerX2 - xOffSet;
	  		leftCornerY2 = centerY2 - yOffSet;
	  		rightCornerX2 = centerX2 + xOffSet;
	  		rightCornerY2 = centerY2 + yOffSet;
	  		//Draw the boxes
	...
	//	float radiusCenterY2 = (meanderWidth / 2 - yCenter);
		...
		smallRadiusIR = meanderSpace/2;
		smallRadiusOR = smallRadiusIR + wireWidth;
		largeRadiusIR = 2*wireWidth + 2*gpSpacing + meanderSpace/2;
		largeRadiusOR = largeRadiusIR + wireWidth;
	  ...
	  {
		LMacro_BindToMenuAndHotKey_v9_30("Tools", NULL /*hotkey*/, "CPW", "CPW", NULL /*hotkey category*/);
	  }
